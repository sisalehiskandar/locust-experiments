...
class ElasticSearchAdapter(BackendAdapter):
    ...
    def __init__(self, elastic_hosts=["http://localhost:9200"], index_name="locust", type_name="stat",
    ...
    def send(self, data):
        ...
        res = self.es.index(index=self.index_name, doc_type=self.type, body=data)
        print(res['result'])
